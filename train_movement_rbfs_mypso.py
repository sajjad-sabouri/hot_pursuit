...
from classes.swarm import PSO_Controller
...
if optimization_weight_function_format == 'rbf':
    configs_file = 'configs/rbf_movement_optimization_mypso.config'
...
with open(configs_file) as f:
    ...
    hyper_parameters = {
        ...
        'w_damper': optimization_config['w_damper']
    }
...
def cost_function(
    ...
    cost = 0
    max_simulation_steps = game_state['max_simulation_steps']
    n_particles = game_state['n_particles']
    initial_coordinates = game_state['initial_coordinates']
    drivers = game_state['drivers']
    polygons = game_state['polygons']
    display_particle_movement = game_state['display_particle_movement']
...
if __name__ == '__main__':
    ...
    # define optimizer
    bounds = {
        'lower_bounds': x_min.reshape(1, -1),
        'upper_bounds': x_max.reshape(1, -1)
    }
    ...
    for suboptimal_iter in range(n_suboptimal_iterations):
        ...
        controller = PSO_Controller(
            ...
            problem_state=game_state
        )

        print(f'Processing new suboptimal solution iteration {suboptimal_iter} ...')
        n_digits = len(str(n_iterations))
        for iter in range(n_iterations):
            print(f'Iteration {str(iter).zfill(len(str(iter)) + n_digits - len(str(iter)))} => ', end='')
            controller.step()
            controller.report()

        best_pos = controller.get_best_pos()
        # controller.plot_cost_history()

        if suboptimal_iter % 2 == 0:
            game_state['display_particle_movement'] = True
            _ = cost_function(
                best_pos.reshape(1, n_var),
                game_state
            )
        else:
            game_state['display_particle_movement'] = False

        drivers.append(
            Gravity_Driver(
                gravity_center=np.array([best_pos[0, 0], best_pos[0, 1]]),
                gravity_intensity_center=np.array([best_pos[0, 2], best_pos[0, 3]]),
                gravity_intensity_variance=np.array([best_pos[0, 4], best_pos[0, 5]]),
            )
        )
